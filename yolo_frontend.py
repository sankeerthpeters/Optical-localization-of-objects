...
from keras.layers import Reshape, Activation, Conv2D, Input, MaxPooling2D, BatchNormalization, Flatten, Dense, Lambda, Dropout, AveragePooling2D
...
class SpecialYOLO(object):
    def __init__(self, input_width,
        ...
        # stack 1
        x = Conv2D(64, (3,3), strides=(1,1), padding='same', name='conv_' + str( num_layer ), use_bias=False)(input_image)  #16
...
##            num_layer += 1
        ...
        # stack 3
        for i in range(0,20):
            x = Conv2D(32, (3,3), strides=(1,1), padding='same', name='conv_' + str(num_layer), use_bias=False)(x)  #32
            x = BatchNormalization(name='norm_' + str(num_layer))(x)
            x = LeakyReLU(alpha=0.1)(x)
            num_layer += 1
        ...
        for i in range(0,15):
            x = Conv2D(16, (3,3), strides=(1,1), padding='same', name='conv_' + str(num_layer), use_bias=False)(x)  #128
            x = BatchNormalization(name='norm_' + str(num_layer))(x)
            x = LeakyReLU(alpha=0.1)(x)
            num_layer += 1
...
##            x = Conv2D(32, (3,3), strides=(1,1), padding='same', name='conv_' + str(num_layer), use_bias=False)(x)  #128
##            x = BatchNormalization(name='norm_' + str(num_layer))(x)
##            x = LeakyReLU(alpha=0.1)(x)
##            num_layer += 1
##
...
##            x = Conv2D(8, (1,1), strides=(1,1), padding='same', name='conv_' + str(num_layer), use_bias=False)(x)  #128
##            x = BatchNormalization(name='norm_' + str(num_layer))(x)
##            x = LeakyReLU(alpha=0.1)(x)
##            x = MaxPooling2D(pool_size=(2, 2))(x)
##            num_layer += 1
...
##        for i in range(0,1):
##            x = Conv2D(256, (1,1), strides=(1,1), padding='same', name='conv_' + str(num_layer), use_bias=False)(x)  #32
##            x = BatchNormalization(name='norm_' + str(num_layer))(x)
##            x = LeakyReLU(alpha=0.1)(x)
##            num_layer += 1   

        x = Conv2D(4+1+ self.nb_class, (3,3), strides=(1,1), padding='same', name='conv_'+str( num_layer ), use_bias=False)(x)
        x = BatchNormalization(name='norm_' + str( num_layer ))(x)
        x = LeakyReLU(alpha=0.1)(x)
        num_layer += 1

        # make the object detection layer
        output = Conv2D(self.nb_kpp * (4 + 1 + self.nb_class),#(x, y, ex, ey, conf, no.of classes)
                        (1,1), strides=(1,1),
                        padding='same',
                        name='DetectionLayer',
        ...
        output = Reshape((self.grid_h, self.grid_w, self.nb_kpp, 4 + 1 + self.nb_class))(x)
        ...
        #--------------------------------------------------------------------------------
        # self.model.load_weights( "shafts.h5" )
        #--------------------------------------------------------------------------------
    ...
    def custom_loss(self, y_true, y_pred):
        # shape of y_pred and y_true: <batch_size> <gridsize_x> <gridsize_y> <1> <5> where last dimension <5> is <x0 y0 ex ey conf (classes as one-hot vector)>
        ...
        # print_op = tf.print( "y_true=", y_true, output_stream=sys.stderr, summarize=-1)
        ...
        # netout must be in image_width and image_height units, i.e. in the range [0...1] over the whole image...really??
        # print_op = tf.print( "y_pred=", y_pred, output_stream=sys.stderr, summarize=-1)
        # with tf.control_dependencies([print_op]):
        #    y_pred = tf.add( y_pred, 0 )
        # mask_shape = tf.shape(y_pred)[:4] # mask_shape is then (batch_size, nb_grid_x, nb_grid_y, 1)
        ...
        pred_exy = (y_pred[..., 2:4])  # predicted unit vectors pointing along part axis in element in last dimension, shape is (batch_size, gridsize_x, gridsize_y, nb_anchors, <2,3 == ex, ey))

        ### extract (=limit to [0...1]) predicted confidence
        pred_kpp_conf = y_pred[...,4]  #tf.sigmoid(y_pred[..., 4]) #predicted keypoint pair confidences shape is (batch_size, gridsize_x, gridsize_y, nb_anchors, <3==confidence>))

        ### extract predicted class probabilities
        pred_kpp_class = y_pred[..., 5:]  # one or more classes (one-hot) starting with element 4 in last dimension,shape is (batch_size, gridsize_x, gridsize_y, <1==nb_anchors>, <3==confidence>)), why tf.argmax is missing?
        ...
        true_exy = y_true[..., 2:4] # x-y-coordinates of y_true, in grid cells units, LUT
        #true_alpha = y_true[..., 2:3] # alpha

        ### keypoint-pair confidence
        true_kpp_conf = y_true[..., 4] # tf.sigmoid(y_true[..., 4]) # confidence
        ...
        true_kpp_class = y_true[..., 5:]  # one hot class vectors
        true_kpp_class_argmax = tf.argmax(y_true[..., 5:], -1) # index of highest class
        ...
        coord_mask = tf.expand_dims(y_true[..., 4], axis=-1) * self.coord_scale  # extract 3rd axis only, this is <nb_anchors>, and append a dimension with size 1 at the end -> (nb_batches, nb_grid_x, nb_grid_y, 1 )
        ...
        # penalize the confidence difference of all keypoints which are reponsible for corresponding ground truth keypoint0, consider y_true[...,4] is either 1 or 0
        conf_mask = conf_mask + y_true[..., 4] * self.object_scale  # set the cells containing keypoints to object_scale, remove last dimension from y_true and keep conf values only

        ### class mask
        # class_mask = y_true[..., 3] * tf.gather(self.class_wt, true_kpp_class_argmax) * self.class_scale
        class_mask = y_true[..., 4] * tf.gather(self.class_wt, true_kpp_class_argmax) * self.class_scale
        ...
        loss_exy       = tf.reduce_sum(tf.square(true_exy-pred_exy) * coord_mask * self.direction_scale) / (nb_coord_kpp + 1e-6) / 2.
        ...
        print_op = tf.print( "loss_conf=", loss_conf , output_stream=sys.stderr)
        with tf.control_dependencies([print_op]):
           loss_conf  = tf.add( loss_conf, 0 )

        loss = tf.cond(tf.less(self.seen, self.warmup_batches+1),
                      lambda: loss_kp0_xy + 10*loss_exy + loss_conf + loss_class, #100, # Adding bias as 10
                      lambda: loss_kp0_xy + 10*loss_exy + loss_conf + loss_class)*10




        if self.debug:
            nb_true_kpp = tf.reduce_sum(y_true[..., 4])
    ...
    def train(self, train_imgs,     # the list of images to train the model
        ...
        image = cv2.imread("D:\\Sankeerth\\THESIS\\Blender data\\128p\\train\\img180cl_000.bmp")
    ...
    def predict(self, image):
        ...
        for kpp in netout_decoded:
            print( kpp.x0, kpp.y0, kpp.Ix, kpp.Iy, kpp.c , kpp.classes )
